...
(function () {
  ...
  if (typeof MutationObserver !== 'undefined') {
    ...
    const oldSetImmediate = util.setImmediate
    util.setImmediate = function (callback) {
      if (Date.now() - now > 15) {
        now = Date.now()
...
function DataBuffer(b, options) {
  ...
  const isArrayBuffer = util.isArrayBuffer(b)
  const isArrayBufferView = util.isArrayBufferView(b)
...
util.DataBuffer.prototype.accommodate = function (amount, growSize) {
  ...
  const dst = new Uint8Array(this.length() + growSize)
  dst.set(src)
...
util.DataBuffer.prototype.toString = function (encoding) {
  const view = new Uint8Array(this.data, this.read, this.length())
...
util.formatNumber = function (number, decimals, dec_point, thousands_sep) {
  ...
  const t = thousands_sep === undefined
    ? '.' : thousands_sep; const
    s = n < 0 ? '-' : ''
...
util.bytesFromIPv6 = function (ip) {
  ...
  for (let i = 0; i < 8; ++i) {
    ...
    const bytes = util.hexToBytes(ip[i])
    if (bytes.length < 2) {
      b.putByte(0)
    }
    b.putBytes(bytes)
...
util.bytesToIPv6 = function (bytes) {
  ...
  for (let i = 0; i < bytes.length; i += 2) {
    let hex = util.bytesToHex(bytes[i] + bytes[i + 1])
...
util.estimateCores = function (options, callback) {
  ...
  function sample(max, samples, numWorkers) {
    if (samples === 0) {
      // get overlap average
      const avg = Math.floor(max.reduce(function (avg, x) {
        return avg + x
      }, 0) / max.length)
